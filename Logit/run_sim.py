...
def experiment(counter, beta0, phi0, A0, lg, n):
    ...
    return lg.w, lg.v, lg.tau, lg.gamma, lg.cv_bs, lg.cv_loss, lg.corr, lg.betas[:,-1]
...
phi = np.stack(t_df.iloc[:, 7].to_numpy())
...
df.to_csv('sim_zeta'+"{:.2f}".format(zeta)+'_theta0'+"{:.2f}".format(theta0)+'.csv', index = False)
