...
rs_df = pd.read_csv('rs'+fmt+'.csv')
sim_df = pd.read_csv('sim'+fmt+'.csv')
...
plt.savefig('estimates_oreder_parameters' + fmt + '.png')
...
ax3.plot(rs_df['alpha'],rs_df['phi'] ,'r-')
ax3.errorbar(sim_df['alpha'],sim_df['phi_mean'],yerr =sim_df['phi_std'],fmt = 'ko', capsize = 3)
ax3.set_ylabel(r"$\hat{\phi}_n$")
ax3.set_xlabel(r'$\alpha$')
ax3.set_xlim( left = 0.0, right = 3.0)

plt.savefig('estimates_metrics' + fmt + '.png')
plt.show()